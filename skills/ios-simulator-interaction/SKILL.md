...
- **"Could not load framework … SimulatorKit"?** iosef uses the Xcode chosen by `DEVELOPER_DIR`, then `xcode-select`, then `/Applications/Xcode.app`. Check which with `iosef status` (the `Xcode:` line), and point it elsewhere with e.g. `DEVELOPER_DIR=/Applications/Xcode-beta.app iosef …` or `sudo xcode-select -s`.
