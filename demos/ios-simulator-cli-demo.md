...
**iosef** is a native Swift CLI that controls iOS Simulator — tapping, typing, swiping, reading the accessibility tree, and capturing screenshots. It talks directly to Apple's private frameworks (IndigoHID, CoreSimulator) for low-latency input injection, making it fast enough for real-time agent loops.
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_view --udid $UDID --output /tmp/showboat_01_initial.png 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_describe_all --udid $UDID 2>/dev/null | head -20
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_tap --x 38 --y 225 --udid $UDID 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_view --udid $UDID --output /tmp/showboat_02_after_tap.png 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_describe_point --x 45 --y 187 --udid $UDID 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_tap --x 222 --y 524 --udid $UDID 2>/dev/null && sleep 0.3 && .build/release/iosef ui_type --text "Hello from showboat\!" --udid $UDID 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_view --udid $UDID --output /tmp/showboat_03_after_type.png 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_swipe --x-start 100 --y-start 458 --x-end 300 --y-end 458 --udid $UDID 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_describe_point --x 327 --y 413 --udid $UDID 2>/dev/null
```
...
```bash
UDID=6C07B68F-054D-434D-B5D7-6C52DCE7D78B && .build/release/iosef ui_view --udid $UDID --output /tmp/showboat_04_final.png 2>/dev/null
```
...
This demo walked through the core agent loop with **iosef**:
...
All commands accept `--udid` for multi-simulator environments. The CLI also runs as an MCP server (`iosef mcp`) for direct agent integration.
